...
Component({
  ...
  methods: {
    ...
    onAuthor() {
      const post = this.data.post as { anonId?: string; authorId?: string; isAnonymous?: boolean } | null;
      if (this.data.anonymous) {
        if (post?.anonId) this.triggerEvent('author', { anonId: post.anonId });
        return;
      }
      if (post?.isAnonymous) {
        this.onTap();
      ...
      wx.navigateTo({
        url: `/pages/user-profile/index?id=${encodeURIComponent(post.authorId)}`,
      });
    },
