...
const EXPECTED_HASHES = {
  ...
  'api/controller/Demand.php': '1B120FCE5DDD7CE3684ED4139024E8644860A7F3C6E5054410295DE5C873B0E1',
};
...
function load(relativePath) {
  ...
  assert(hash === EXPECTED_HASHES[relativePath], `${relativePath} SHA-256 does not match the audit record`);
...
assert(
  /Route::rule\(\s*'internal\/sync\/tutor-demands'\s*,\s*'Demand\/syncTutorDemands'\s*,\s*'GET'\s*\)/.test(route),
  'GET /internal/sync/tutor-demands route is missing',
);
assert(method.includes("getenv('TUTOR_SYNC_TOKEN')"), 'TUTOR_SYNC_TOKEN environment lookup is missing');
assert(method.includes("header('X-Sync-Token'"), 'X-Sync-Token header lookup is missing');
assert(method.includes('hash_equals($expectedToken, $providedToken)'), 'constant-time token comparison is missing');
